...
function applySpectralLayer(fields, track, reasons, sources) {
  const g = track.glyph || track.glyphFeatures;
  if (!g || typeof g !== 'object') return { fields, reasons };
  const patch = {};
  if (!fields.genre && g.genreHint) patch.genre = g.genreHint;
  if (!patch.genre) return { fields, reasons };
  sources.push('glyph-spectral');
  const bpm = g.bpm ? `${g.bpm} BPM` : '';
  const src = g.bpmSource ? ` (${g.bpmSource})` : '';
  return mergeFields(
    { ...fields, _reasons: reasons },
    patch,
    [...reasons, `glyph-spectral: genre from audio${bpm ? ` · ${bpm}${src}` : ''}`],
    'glyph-spectral'
  );
}
...
export async function runGlyphPipeline(track, state, baseResult, { libraryRows = [], settings = {} }) {
  ...
  const spectralMerged = applySpectralLayer(fields, track, reasons, sources);
...
export async function analyzeTrackFull(track, state, options = {}) {
  ...
  const inputWithSpectral = {
    ...input,
    context: {
      ...(input.context || {}),
      glyphFeatures: input.context?.glyphFeatures || track.glyph || null,
    },
  };

  let result = await analyze(inputWithSpectral, {
    provider: 'mi',
    tryLocal: false,
